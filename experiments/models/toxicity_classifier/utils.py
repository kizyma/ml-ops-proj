...
# binary classification overfits like there is no tommorow
...
def build_classifier_multilabel(transformer_name: str, max_len: int, lr: float) -> tf.keras.Model:
    """
...
def build_classifier(transformer_name: str, max_len: int, lr: float) -> tf.keras.Model:
    ...
    dropout_layer = tf.keras.layers.Dropout(0.3)(sequence_output)

    # Use Global Average Pooling
    pooled_output = tf.keras.layers.GlobalAvgPool1D(name="AvePool")(
        dropout_layer, mask=attention_mask
    )

    # L2 Regularization can be added to the Dense layer
    l2_regularizer = l2(1e-5)
    cls_layer = tf.keras.layers.Dense(
        units=1,
        activation="sigmoid",
        kernel_initializer=tf.keras.initializers.GlorotNormal(seed=generate_random_seed()),
...
def build_classifier_v2(transformer_name: str, max_len: int, lr: float) -> tf.keras.Model:
    word_ids = tf.keras.layers.Input(shape=(max_len,), dtype=tf.int32, name="base_word_ids")
    attention_mask = tf.keras.layers.Input(shape=(max_len,), dtype=tf.int32, name="base_attention_mask")
    ...
    # Apply dropout
    dropout_layer = tf.keras.layers.Dropout(0.5)(sequence_output)

    # Masking: Set padding positions to a large negative value
    mask_expanded = tf.cast(tf.expand_dims(attention_mask, -1), tf.float32)
    masked_sequence = dropout_layer + (1.0 - mask_expanded) * -1e9

    # Use Global Max Pooling (without an explicit mask argument)
    pooled_output = tf.keras.layers.GlobalMaxPool1D(name="MaxPool")(masked_sequence)
    ...
    cls_model = tf.keras.Model(inputs=[word_ids, attention_mask], outputs=cls_layer, name="ToxicityClassifier")
    cls_model.compile(optimizer=tfa.optimizers.AdamW(learning_rate=lr, weight_decay=1e-5), loss="binary_crossentropy")
